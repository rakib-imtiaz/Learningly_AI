...
const FeatureCard = ({ icon: Icon, title, description, color, shadowColor, image }: (typeof features)[0]) => (
  ...
  >
    <Card className={cn(
      ...
      <div className="relative h-40 w-full mb-6 rounded-lg overflow-hidden bg-gradient-to-br from-electric-blue/10 to-purple/10 group">
        {image && (
          <Image
            src={image}
            alt={title}
            fill
            className="object-contain p-2 opacity-0 group-hover:opacity-100 transition-all duration-500 scale-95 group-hover:scale-100"
          />
        )}

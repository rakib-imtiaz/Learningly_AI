...
function createErrorResponse(error: CallbackError, origin: string): NextResponse {
  ...
  return NextResponse.redirect(`${origin}/dashboard?${errorParams.toString()}`)
}
...
export async function GET(request: NextRequest) {
  ...
  // Note: Supabase automatically validates the state parameter when using createServerClient
  // This additional check provides an extra layer of security
  if (!params.state) {
    logOAuthEvent('error', {
